...
export function EditorView({
  ...
  useEffect(() => {
    let seen = controller.draft.body,
      supported = true;
    ...
    const instance = createEditor(
      ...
      (body, meaningful) => {
        seen = body;
        controller.change(body, meaningful);
      },
      (value) => controller.composition(value),
    );
    ...
    const display = (body: string) => {
      ...
      if (supported) {
        setRaw(null);
        instance.setEditable(true, false);
        instance.commands.setContent(body, { contentType: 'markdown', emitUpdate: false });
      } else {
        instance.setEditable(false, false);
        instance.commands.setContent('', { emitUpdate: false });
        setRaw(body);
      }
      performance.clearMeasures('pmem:markdown');
    ...
    performance.clearMeasures('pmem:editor');
    performance.measure('pmem:editor', { start: initializing, end: performance.now() });
    const frame = requestAnimationFrame(() => {
      performance.clearMarks('pmem:ready');
      performance.mark('pmem:ready');
      document.documentElement.dataset.ready = controller.draft.id;
    });
    return () => {
  ...
  const run = (action: (e: Editor) => unknown) => {
    const e = editor.current;
    if (e?.isEditable && !e.view.composing) action(e);
  };
  return (
    <>
      <div className="toolbar" aria-label="文字格式">
        <select
          ...
          value=""
          disabled={raw !== null}
          onChange={(event) => run((e) => actions[Number(event.target.value)][1](e))}
        >
          ...
          onClick={() =>
            run((e) => {
              const href = window.prompt(
                ...
                e.getAttributes('link').href ?? 'https://',
              );
              if (href === '') e.chain().focus().unsetLink().run();
              else if (href && safeLink(href)) e.chain().focus().setLink({ href }).run();
            })
          }
        >
          链接
      ...
      {raw !== null && (
        <div className="raw-note">
          <p>这篇笔记包含暂不支持的格式，已保留原文。你可以查看、复制，或另存后继续记录。</p>
