...
export class NoteController {
  ...
  view(): ViewState {
    const messages = {
      ...
      auth: '登录已过期，请重新登录',
      conflict: '其他设备修改了这篇笔记，请选择如何保留内容',
      deleted: '这篇笔记已被删除，当前草稿仍保留',
      pending: '保存结果待确认，当前草稿仍保留',
    };
    return {
      body: this.draft.body,
      dirty: this.dirty,
      block: this.draft.block,
      remote: this.remote,
      deleted: this.deleted,
      localFailed: this.localFailed,
      status: this.localFailed
        ? '本机保存失败，请复制内容'
        : this.draft.block
          ? `${messages[this.draft.block]}${this.dirty && this.localRev === this.draft.rev ? ' · 已保存到本机' : ''}`
          : this.localRev !== this.draft.rev
            ? '正在保存'
            : this.dirty
              ? '已保存到本机，待同步'
              : this.draft.base
  ...
  change(body: string, meaningful: boolean) {
    ...
    this.draft.meaningful = meaningful;
    this.draft.rev++;
    this.draft.modified = Date.now();
    this.dirty = true;
    ...
    void this.persist()
      .then(() => {
        if (!this.draft.base && !meaningful && !this.draft.pending && this.draft.rev === revision)
          return this.db.remove(this.draft.key, revision);
      })
      .catch(() => {});
  ...
  async shutdown() {
    this.stopped = true;
    this.cancelTimers();
    while (this.inFlight || this.polling) await new Promise((resolve) => setTimeout(resolve, 10));
  ...
  async save() {
    ...
    const sent: Submission = {
      ...
      rev: this.draft.rev,
      kind: this.draft.base ? 'replace' : 'create',
    };
    ...
    try {
      const response = await this.api.call(`/notes/${this.draft.id}`, {
        method: sent.kind === 'create' ? 'POST' : 'PUT',
        headers: {
          'content-type': 'text/markdown',
          ...(sent.base ? { 'if-match': sent.base } : {}),
        },
        body: sent.body,
      });
      const etag = response.headers.get('etag');
      if (!etag) throw new Error('Missing save acknowledgment');
    ...
    if (this.dirty && !this.draft.block)
      this.timer = setTimeout(() => {
        void this.save();
      }, 500);
  }
  private async ack(sent: Submission, etag: string) {
    ...
    this.draft.block = undefined;
    try {
      if (this.draft.rev === sent.rev) {
        await this.queue.catch(() => {});
        await this.db.clean(this.draft.namespace, this.draft.id, sent.body, etag, {
          key: this.draft.key,
          rev: sent.rev,
        });
  ...
  private async failure(error: unknown) {
    if (
      ...
      [400, 401, 403, 404, 409, 412, 413, 415, 422, 428, 429].includes(error.status)
    ) {
      if (error.status === 409) {
  ...
  private async reconcile(rejectedCreate = false) {
    ...
    try {
      ...
      if (body === sent.body && sent.kind !== 'delete' && etag) await this.ack(sent, etag);
        ...
        this.draft.block = rejectedCreate ? 'conflict' : 'pending';
        if (rejectedCreate) this.draft.pending = undefined;
        this.remote = body;
        await this.persist();
      }
    } catch (error) {
      if (error instanceof HttpError && error.status === 404 && sent.kind === 'delete') {
  ...
  async refresh() {
    ...
    try {
      ...
      if (!etag) return;
      if (this.draft.base !== baseline || this.draft.rev !== revision) return;
      if (this.dirty || this.composing || this.inFlight) {
        this.remote = body;
        this.draft.block = 'conflict';
        await this.persist();
      } else {
        this.draft.body = body;
        this.draft.base = etag;
      ...
      if (error instanceof HttpError && [401, 404].includes(error.status)) {
        this.draft.block = error.status === 401 ? 'auth' : 'deleted';
        if (this.draft.block === 'auth') this.onAuth();
        if (this.dirty) await this.persist().catch(() => {});
      }
    } finally {
      this.polling = false;
      this.emit();
      if (this.dirty && !this.draft.block) void this.save();
    }
  }
  async useRemote() {
    ...
    await this.queue.catch(() => {});
    if (
      this.draft.rev !== revision ||
      this.inFlight ||
      this.draft.pending ||
      this.composing ||
      this.stopped
    )
      throw new Error('Draft changed while reading remote');
    this.draft.body = body;
    this.draft.base = etag;
    this.draft.rev++;
    this.draft.retry = undefined;
    ...
    const job = this.queue.then(() =>
      this.db.clean(this.draft.namespace, this.draft.id, body, etag, {
        key: this.draft.key,
        rev: revision,
      }),
    );
  ...
  private async finishDelete() {
    ...
    if (this.draft.pending && this.draft.rev !== this.draft.pending.rev) {
      this.draft.pending = undefined;
      this.draft.block = 'deleted';
      await this.persist();
    } else {
      this.deleted = true;
      await this.db.remove(this.draft.key);
      this.dirty = false;
      this.draft.pending = undefined;
    }
    this.onSynced();
  ...
  async remove() {
    ...
    try {
      await this.persist();
      await this.api.call(`/notes/${this.draft.id}`, {
        method: 'DELETE',
        headers: { 'if-match': this.draft.base },
      });
      await this.finishDelete();
      return this.deleted;
    } catch (error) {
