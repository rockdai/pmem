...
try {
  ...
  let stopping = false;
  const close = async () => { if (stopping) return; stopping = true; await app.close(); process.exit(0); };
  process.on('SIGTERM', close); process.on('SIGINT', close);
} catch (error) {
  // SDK errors may include credentials/request bodies: log only a bounded diagnostic.
  console.error('Startup failed:', error instanceof Error && !('requestId' in error) ? error.message.split('\n')[0] : 'OSS access failed; verify RAM permissions, connectivity and state volume');
  process.exitCode = 1;
}
