...
vi.mock('node:fs/promises', async (importOriginal) => ({
  ...
  open: vi.fn((...args: Parameters<typeof fs.open>) => actualOpen(...args)),
  rename: vi.fn((...args: Parameters<typeof fs.rename>) => actualRename(...args)),
}));
const { open: actualOpen, rename: actualRename } =
  await vi.importActual<typeof fs>('node:fs/promises');
let root: string;
beforeEach(async () => {
  root = await fs.mkdtemp(join(tmpdir(), 'pmem-atomic-'));
});
afterEach(async () => {
  vi.mocked(fs.open).mockImplementation(actualOpen);
  vi.mocked(fs.rename).mockImplementation(actualRename);
...
it('a full disk during writing preserves the original file and removes the temporary file', async () => {
  ...
  await expect(atomicFile(root, 'note.md', Buffer.from('new'))).rejects.toMatchObject({
    code: 'ENOSPC',
  });
  expect(await fs.readFile(join(root, 'note.md'), 'utf8')).toBe('old');
...
it('failed publication and exclusive creation cannot replace an existing note', async () => {
  ...
  await expect(atomicFile(root, 'note.md', Buffer.from('new'), true)).rejects.toMatchObject({
    code: 'EEXIST',
  });
  expect(await fs.readFile(join(root, 'note.md'), 'utf8')).toBe('old');
  expect(await fs.readdir(root)).toEqual(['note.md']);
...
it('startup only removes regular temporary files with the application UUID naming scheme', async () => {
  ...
  await fs.writeFile(join(root, name), 'incomplete');
  await fs.writeFile(join(root, '.tmp-user-file'), 'keep');
  await fs.symlink('/etc/passwd', join(root, `.tmp-${crypto.randomUUID()}`));
  await cleanTemporary(root);
  const names = await fs.readdir(root);
  expect(names).not.toContain(name);
  expect(names).toContain('.tmp-user-file');
