...
export async function createApp(config: Config, store: Store, webRoot = resolve('dist/web')) {
  ...
  app.addContentTypeParser('text/markdown', { parseAs: 'buffer' }, (_req, body, done) =>
    done(null, body),
  );
  app.addHook('onSend', async (request, reply) => {
    ...
    if (request.routeOptions.url?.startsWith('/api/') || request.url === '/healthz')
  ...
  app.addHook('preHandler', async (request, reply) => {
    ...
    if (mutation && request.headers.origin !== config.origin)
      throw new AppError(403, 'origin_rejected');
    if (login) return;
    if (
      !equal(request.session.get('authBinding'), authBinding) ||
      request.session.get('account') !== config.account
    ) {
  ...
  app.post<{ Body: { account?: string; password?: string } }>(
    '/api/v1/auth/login',
    { bodyLimit: 4096, config: { rateLimit: { max: 10, timeWindow: 15 * 60_000 } } },
    async (request) => {
      ...
      request.session.set('csrf', randomBytes(24).toString('hex'));
      return {
  ...
  app.get<{ Querystring: { cursor?: string; limit?: string } }>('/api/v1/notes', async (request) =>
    notes.list(
    ...
    app.route<{ Params: { id: string }; Body: Buffer }>({
      ...
      handler: async (request, reply) => {
        if (
          ...
          !request.headers['content-type']?.startsWith('text/markdown')
        )
          throw new AppError(415, 'markdown_required');
        const etag = await notes.write(
          request.params.id,
          request.body,
          method === 'POST' ? 'create' : 'replace',
  ...
  app.delete<{ Params: { id: string } }>('/api/v1/notes/:id', async (request, reply) => {
    await notes.remove(request.params.id, request.headers['if-match']);
    reply.status(204).send();
  });
  if (existsSync(webRoot)) {
    await app.register(staticFiles, {
      ...
      setHeaders(reply, path) {
        if (/index\.html(?:\.gz|\.br)?$/.test(path)) reply.header('Cache-Control', 'no-cache');
      },
    });
  }
  return app;
